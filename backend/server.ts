...
// Clean production-ready logging middleware
app.use((req, res, next) => {
  const timestamp = new Date().toISOString();

  // Skip health checks and root requests (Render health checks)
  ...
  if (req.originalUrl.startsWith("/api/")) {
    ...
    // Only log auth issues, not every request
    if (
      !req.headers.authorization &&
      req.originalUrl !== "/api/auth/login" &&
      req.originalUrl !== "/api/auth/signup"
    ) {
      console.log("⚠️ Missing authorization header for protected route");
    }
  }
